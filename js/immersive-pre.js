...
import { resonanceScene } from "./util/spatial-audio.js";
...
window.parseSpeech = (pattern, action) => {
  ...
  for (let n = 0; n < pattern.length; n++) find(pattern[n]);
  if (i < words.length) {
    action(vars);
    window.speech = "";
  }
...
async function processAudioData(buffer, callback) {
  ...
  const res = await fetch("/api/wit/speech", {
    ...
    headers: {
      "content-type":
        "audio/raw;encoding=floating-point;bits=32;rate=44100;endian=little",
  ...
  if (!window.audioContext) {
    ...
    let onSuccess = (stream) => {
      ...
      scriptProcessor.onaudioprocess = (e) => {
        ...
        if (audioBuffer.length < 20 * 44100) {
          // 44100 samples = 1 second, max 20 seconds
...
function initWebcam() {
  if (!isHeadset) {
    videoElement.innerHTML =
      '<video id=webcam autoplay=true style="position:absolute;left:-2560px;"></video>';
  ...
  setInterval(function () {
    var robotCarCmd = {
      cmd: [
        ...
        {
          id: window.robotCarID[1],
          parameter: window.robotCarCmd[window.robotCarID[1]],
        },
        {
          id: window.robotCarID[2],
          parameter: window.robotCarCmd[window.robotCarID[2]],
        },
        {
          id: window.robotCarID[3],
          parameter: window.robotCarCmd[window.robotCarID[3]],
        },
      ],
    };
...
function initGL() {
  ...
  global
    .scene()
    .inputRenderer.setControllerMesh(
  ...
  window.clay.renderingIsActive = !window.containsFlags(
    window.presentationMode,
    window.PRESENTATION_FLAG_NO_RENDER
  );
}
...
async function onRequestSession() {
  ...
  try {
    window.persistentMicStream = await navigator.mediaDevices.getUserMedia({
      audio: true,
    });
    ...
    console.error("❌ Error message:", error.message);
    alert(
      "⚠️ Microphone access denied or unavailable!\n\nError: " +
        error.message +
...
async function onSessionStarted(session) {
  ...
  let onRequestRefSpace = (refSpace) => {
    ...
    try {
      if (typeof clay !== "undefined" && clay.vrWidgets) {
        window.xrHandLabel = clay.vrWidgets
          .add("label")
          .info("index: n/a")
          .move(0, 2.2, -0.5)
          .scale(0.05);
  ...
  session
    .requestReferenceSpace(refSpaceType)
    .then(onRequestRefSpace)
    .catch(() => {
      refSpaceType = "local-floor";
      session.requestReferenceSpace(refSpaceType).then(onRequestRefSpace);
    });
}
...
function onXRFrame(t, frame) {
  if (!window.SpeechRecognition && isSpeechRecognitionEnabled) initWebSpeech();
  ...
  // if (window.playerid) {
  const thisAvatar = window.avatars[window.playerid];
  const seenControllerHands = { left: false, right: false };
  for (let source of session.inputSources) {
    if (source.handedness && source.gamepad) {
      if (source.handedness === "left" || source.handedness === "right")
        seenControllerHands[source.handedness] = true;
  ...
  // If a controller hand disappears this frame, clear its cached input/matrix
  // so beams and button edges don't latch on stale state.
  for (const hand of ["left", "right"]) {
    if (seenControllerHands[hand]) continue;
    controllerMatrix[hand] = [];
    joyStickState[hand] = { x: 0, y: 0 };
    for (let i = 0; i < buttonState[hand].length; i++)
      buttonState[hand][i] = { pressed: false, touched: false, value: 0 };
  }
  // }
    ...
    if (inputSource.hand) {
      ...
      if (indexTip) {
        ...
        if (pose) {
          ...
          window.latestHandPos[handKey].indexTip = {
            x: p.x,
            y: p.y,
            z: p.z,
            t: time,
          ...
          if (window.xrHandLabel) {
            try {
              window.xrHandLabel.info(
                `index: ${p.x.toFixed(3)}, ${p.y.toFixed(3)}, ${p.z.toFixed(3)}`
              );
            } catch (e) {}
          }
...
window.addProperties = (instance, setAction) => {
  ...
  instance.setProperty = (name, value) => {
    ...
    if (setAction) setAction(name, value);
  };
};
