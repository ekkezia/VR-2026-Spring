	...
	const ANTICIPATION_TIME = 1;
		...
		if (state.started && state.activeSongIdx !== null) {
			...
			for (let i = 0; i < lyricNodes[state.activeSongIdx].length; i++) {
				...
				const { spawnTime, beatTime, position, color: rgb, hit, node } = nodeData;
		...
		for (let i = 0; i < menuGroup.nChildren(); i++) {
			...
			if (uvdL) {
				...
				if (state.menuBtnPressed) {
					...
					// Lazy load audio and lyrics for selected song
					if (!state.audioElement[i]) {
						state.audioElement[i] = new Audio(songs[i].url);
						state.audioElement[i].crossOrigin = 'anonymous';
					}
					if (!state.lyrics[i]) {
						buildLyrics(songs[i].beatUrl).then(lyrics => {
							state.lyrics[i] = [...lyrics];
							// Build lyric nodes for this song
							const songNodes = [];
							for (let j = 0; j < lyrics.length; j++) {
								...
								songNodes.push({
									idx: j,
									position: [lyrics[j].x, lyrics[j].y, LYRIC_Z],
									opacity: 0.3,
									beatTime: beatTime,
									spawnTime: spawnTime,
									color: lyrics[j].rgb,
							...
							lyricNodes[i] = songNodes;
							console.log('Loaded lyrics for song:', songs[i].title, lyrics.length, state.lyrics, lyricNodes.length);
						});
					}
					// Immediately pause and reset all other audio elements
					for (let j = 0; j < state.audioElement.length; j++) {
						if (j !== i && state.audioElement[j]) {
			...
			if (lyricNodes[state.activeSongIdx]) {
				...
				for (let i = 0; i < lyricNodes[state.activeSongIdx].length; i++) {
                    // console.log('Checking lyric idx:', i, 'hit status:', lyricNodes[state.activeSongIdx][i].hit, lyricNodes[state.activeSongIdx][i].node);
					let beatR = beamR.hitRect(lyricNodes[state.activeSongIdx][i].node.getGlobalMatrix());
                    // console.log('global matrix', lyricNodes[state.activeSongIdx][i].node.getGlobalMatrix())
			...
			if (uvdR) {
				...
				let u = uvdR[0],
					v = uvdR[1];
				vibrate('right', u*u < .033 && v*v < .09 ? 1 : .4);
				model
					.add('diskZ')
