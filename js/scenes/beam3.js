...
export const init = async (model) => {
	...
	model.move(0, -0.2, 0).animate(() => {
		console.log('beam pos', beamL, beamL);
		beamL.update();
		beamR.update();

		// beamLDisplay.identity().move(beamL.m[12], beamL.m[13], beamL.m[14]);
		...
		for (let i = 0; i < menuGroup.nChildren(); i++) {
			...
			// const tip = [
			// 	beamRDisplay.getGlobalMatrix()[12],
			// 	beamRDisplay.getGlobalMatrix()[13],
			// 	beamRDisplay.getGlobalMatrix()[14],
			// ];
			// const isHit = cg.isPointInBox(tip, menu.getGlobalMatrix());
