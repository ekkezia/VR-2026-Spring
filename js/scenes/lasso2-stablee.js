...
export const init = async (model) => {
  ...
  function upsertPartnerBoardItem(area) {
    ...
    if (index >= 0) {
      ...
      items.push({
        ...
        w: drawW,
        h: drawH,
        x: 90 + ((offset * 36) % Math.max(120, PARTNER_CANVAS_WIDTH - 180)),
        y: 110 + ((offset * 28) % Math.max(120, PARTNER_CANVAS_HEIGHT - 180)),
        layer: maxLayer + 1,
        createdAt: nowMs(),
        updatedAt: nowMs(),
      });
  ...
  function isAreaCenterOverComputerCanvas(area) {
    if (!computerCanvasHitVolume) return false;
    const center = getAreaCenter(area);
    if (!center) return false;
    const canvasMatrix = computerCanvasHitVolume.getGlobalMatrix();
    if (!canvasMatrix) return false;
    return cg.isPointInBox(center, canvasMatrix);
  }

  function setPartnerPlacementArea(area) {
    if (!area || !hasAreaTexture(area)) return;
    const wasPlaced = !!area.partnerPlaced;
    area.partnerPlaced = true;
    area.projectedToComputerCanvas = true;
    if (!wasPlaced) upsertPartnerBoardItem(area);
  }

  function updateAreaComputerCanvasProjection(area) {
    const isDraggedArea =
      !!dragging && dragging.type === "area" && dragging.area === area;
    const isInsideDropZone = isAreaCenterOverComputerCanvas(area);
    ...
    if (isDraggedArea) {
      ...
      area.projectedToComputerCanvas =
        (isInsideDropZone && hasAreaTexture(area)) || area.partnerPlaced;
      return;
    }

    if (isInsideDropZone && hasAreaTexture(area)) {
      setPartnerPlacementArea(area);
      area.projectedToComputerCanvas = true;
      return;
    }
