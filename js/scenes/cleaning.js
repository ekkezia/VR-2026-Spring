import * as cg from '../render/core/cg.js';
import { ControllerBeam } from '../render/core/controllerInput.js';

// INTERSECT CONTROLLER BEAMS WITH A RECTANGLE.
const SFX_URL = '/media/sound/bounce/2.wav';
let sfxBuffer = null;

let currentScore = 0;
const y = 1.5;
const COMMENT_TEXT_SCALE = 5;

const createAudioContext = () =>
    new (window.AudioContext || window.webkitAudioContext)();
const state = {
    audioContext: createAudioContext(),
    audioElement: [],
    audioBuffer: [],
    started: false,
    pressed: false,
    released: false,
    lyrics: [],
    loading: false,
    activeSongIdx: null,
    currentBeatIdx: 0,
    currentScore: 0,
    commentTimer: -999,
    beamDisplayPos: {
        left: [0, 0, 0],
        right: [0, 0, 0],
    },
    beamDisplayRot: {
        left: [0, 0, 0],
        right: [0, 0, 0],
    },
    calibrationStatus: {
        topLeft: 'uncalibrated',
        bottomRight: 'uncalibrated',
    }, // | 'calibrating' | 'calibrated'
	surface: {
		topLeft: [0, 0, 0],
		bottomRight: [0, 0, 0],
	}
};
const mixColor = (base, amount) => [
    base[0] + (1 - base[0]) * amount,
    base[1] + (1 - base[1]) * amount,
    base[2] + (1 - base[2]) * amount,
];

export const init = async (model) => {
    const inch = 0.0254,
        y = 1;
    let round = (t) => ((t + '').charAt(0) == '-' ? '' : ' ') + cg.round(t);
    let beamL = new ControllerBeam(model, 'left');
    let beamR = new ControllerBeam(model, 'right');

    // Preload SFX buffer
    try {
        const sfxResponse = await fetch(SFX_URL);
        const sfxArrayBuffer = await sfxResponse.arrayBuffer();
        sfxBuffer = await state.audioContext.decodeAudioData(
            sfxArrayBuffer.slice(0),
        );
    } catch (err) {
        console.error('Failed to load SFX:', err);
    }

    const lyricNodes = [];

    // All beat nodes live under this group. Its matrix IS the surface frame,
    // so setting it once per frame orients every beat correctly for any surface angle.
    const beatGroup = model.add();

    const buildLyrics = async (path) => {
        try {
            const lyricsResponse = await fetch(path);
            const lyrics = await lyricsResponse.json();
            return lyrics;
        } catch (err) {
            console.error('Failed to load lyrics:', err);
        }
    };

    // menu
    // Load songs.json
    const response = await fetch('/media/sound/json/songs.json');
    const songs = await response.json();

    // Preload all audio files and store in a map
    for (let i = 0; i < songs.length; i++) {
        const song = songs[i];
        console.log('Preloading audio for song:', song.title);
        try {
            state.audioElement.push(new Audio(song.url));
            state.audioElement[state.audioElement.length - 1].crossOrigin =
                'anonymous';

            console.log('Loaded Audio Element', state.audioElement);
            state.loading = false;

            const lyrics = await buildLyrics(song.beatUrl);
            state.lyrics.push([...lyrics]);
            const beatDuration = 60 / song.bpm; // one beat period in seconds
            const gridCols = Math.ceil(Math.sqrt(lyrics.length));
            const gridRows = Math.ceil(lyrics.length / gridCols);

            // pre create the lyrics, but just show them as opacity 0 first, then we can just update the opacity on each frame to show/hide them with a fade
            // Random x/y within bounds for each lyric
            const songNodes = [];
            for (let j = 0; j < lyrics.length; j++) {
                const beatTime = lyrics[j].time;
                const spawnTime = beatTime - beatDuration; // approach window = exactly one beat
                songNodes.push({
                    idx: j,
                    u: null, // assigned once surface is calibrated
                    v: null,
                    beatTime,
                    spawnTime,
                    beatDuration,
                    color: lyrics[j].rgb,
                    hit: false,
                    node: beatGroup
                        .add('square')
                        .color(lyrics[j].rgb[0], lyrics[j].rgb[1], lyrics[j].rgb[2])
                        .scale(0),
                });
            }

            lyricNodes.push(songNodes);
            console.log(
                'Loaded lyrics for song:',
                song.title,
                lyrics.length,
                state.lyrics,
                lyricNodes.length,
            );
        } catch (err) {
            console.error(err);
        }
    }

    console.log('Initialized scene with lyrics:', model.nChildren(), lyricNodes);

    // Create a menu group to hold all song menus
    const menuGroup = model.add();

    // Layout parameters
    const startY = 2.0;
    const gapY = 0.4;

    for (let i = 0; i < songs.length; i++) {
        const song = songs[i];
        // Each menu is a square with the song title
        const menu = menuGroup
            .add('square')
            .move(0, startY - i * gapY, -0.5)
            .scale(0.2, 0.2, 0.01)
            .color(0.25, 0.35, 0.5);
        menu
            .add(clay.text(song.title))
            .move(-0.3, -0.05, 0.01)
            .color(0, 0, 0)
            .scale(6);
        // Optionally, add interaction logic here
    }

    const scoreboard = model
        .add('square')
        .move(-0.8, 2.2, -0.5)
        .scale(0.1, 0.1, 0.01)
        .color(0.25, 0.35, 0.5);
    scoreboard.add(clay.text('Score')).move(-0.5, 0.5, 0.01).scale(10);

    const surfaceRectNode = model.add('square').color(1, 0, 1).opacity(0.7).dull(); 
    const tlPoint = model.add('diskZ').color(1, 0, 1).opacity(0.8).scale(0.02).dull();
    const brPoint = model.add('diskZ').color(1, 0, 1).opacity(0.8).scale(0.02).dull();

    const quitBtn = model
        .add('diskZ')
        .move(-0.8, 2, -0.5)
        .scale(0.1)
        .color(1, 0, 0);
    quitBtn
        .add(clay.text('QUIT'))
        .move(-0.3, -0.05, 0.01)
        .color(1, 1, 1)
        .scale(6);

    let COMMENT = {
        PERFECT: 'PERFECT!',
        GREAT: 'GREAT',
        MISSED: 'MISSED',
        NONE: '',
    };

    const commentText = model.add();

    const beamRPointer = model.add();
    beamRPointer.add('diskZ').scale(0.01).color(0, 1, 0).dull();

    inputEvents.onMove = () => {};
    
    inputEvents.onPress = (hand) => {
        state.pressed = true;
    };

    inputEvents.onRelease = (hand) => {
        state.pressed = false;
        // console.log('Menu button released, starting game');
        state.released = true;
        setTimeout(() => {
            state.released = false;
        }, 100); // small delay to differentiate between quick tap and hold
    };

    // Match generate_beat_timings.py scene bounds minus margins
    const X_RANGE = (1.5 - (-1.5)) - 2 * 0.15; // 2.7
    const Y_RANGE = (2.8 - 1.4)   - 2 * 0.05;  // 1.3
    // BEAT_SCALE is now computed per-song from grid dimensions (see songScales)

    const BEAT_SURFACE_OFFSET = 0.02; // metres above the surface
    let beatsPositioned = false;     // random (u,v) assigned once after calibration

    const LOCK_DURATION  = 3;    // seconds to hold still before locking
    const LOCK_THRESHOLD = 0.03; // meters — movement beyond this resets the timer
    const holdState = {
        left:  { anchorPos: null, startTime: null },
        right: { anchorPos: null, startTime: null },
    };

    model.animate(() => {
        const tlCalibrated = state.calibrationStatus.topLeft    === 'calibrated';
        const brCalibrated = state.calibrationStatus.bottomRight === 'calibrated';
        const bothCalibrated = tlCalibrated && brCalibrated;

        // Left hand tracks topLeft corner
        if (!tlCalibrated) {
            const lPos = clientState.finger(clientID, 'left', 1);
            if (lPos) {
                state.surface.topLeft = lPos;
                const hs = holdState.left;
                if (hs.anchorPos && cg.distance(lPos, hs.anchorPos) < LOCK_THRESHOLD) {
                    const progress = Math.min((model.time - hs.startTime) / LOCK_DURATION, 1);
                    tlPoint.opacity(progress);
                    if (progress >= 1) state.calibrationStatus.topLeft = 'calibrated';
                } else {
                    holdState.left = { anchorPos: lPos.slice(), startTime: model.time };
                    tlPoint.opacity(0);
                }
            }
        }

        // Right hand tracks bottomRight corner
        if (!brCalibrated) {
            const rPos = clientState.finger(clientID, 'right', 1);
            if (rPos) {
                state.surface.bottomRight = rPos;
                const hs = holdState.right;
                if (hs.anchorPos && cg.distance(rPos, hs.anchorPos) < LOCK_THRESHOLD) {
                    const progress = Math.min((model.time - hs.startTime) / LOCK_DURATION, 1);
                    brPoint.opacity(progress);
                    if (progress >= 1) state.calibrationStatus.bottomRight = 'calibrated';
                } else {
                    holdState.right = { anchorPos: rPos.slice(), startTime: model.time };
                    brPoint.opacity(0);
                }
            }
        }

        // Draw rectangle between the two corners
        const tl = state.surface.topLeft;
        const br = state.surface.bottomRight;
        const cx = (tl[0] + br[0]) / 2;
        const cy = (tl[1] + br[1]) / 2;
        const cz = (tl[2] + br[2]) / 2;
        const dx = br[0] - tl[0];
        const dy = br[1] - tl[1];
        const dz = br[2] - tl[2];
        const w    = Math.abs(dx);                        // width  = X span
        const yLen = Math.sqrt(dy * dy + dz * dz);
        const h    = yLen;                                // height = YZ span

        // Width axis: always world X.
        // Height axis: the direction of (0, dy, dz) — this naturally tilts the
        //   rect from vertical (wall, dz≈0) to horizontal (table, dy≈0).
        // Normal: cross of the two axes.
        const xDir = [1, 0, 0];
        const yDir = yLen > 1e-4 ? [0, dy / yLen, dz / yLen] : [0, 1, 0];
        const nDir = cg.cross(xDir, yDir);
        const rectMat = [
            xDir[0]*w/2, xDir[1]*w/2, xDir[2]*w/2, 0,
            yDir[0]*h/2, yDir[1]*h/2, yDir[2]*h/2, 0,
            nDir[0],     nDir[1],     nDir[2],     0,
            cx,          cy,          cz,          1,
        ];
        surfaceRectNode.setMatrix(rectMat)
            .color(bothCalibrated ? 0 : 1, bothCalibrated ? 1 : 0, bothCalibrated ? 0 : 1);
        tlPoint.identity().move(tl[0], tl[1], tl[2]).scale(0.04)
            .color(tlCalibrated ? 0 : 1, tlCalibrated ? 1 : 0, tlCalibrated ? 0 : 1);
        brPoint.identity().move(br[0], br[1], br[2]).scale(0.04)
            .color(brCalibrated ? 0 : 1, brCalibrated ? 1 : 0, brCalibrated ? 0 : 1);

        // Orient the beat group to the surface frame (unit-length axes + offset along normal).
        // Beats express their position in local (u, v) space; this one setMatrix call
        // rotates and translates all of them correctly regardless of surface angle.
        const beatGroupMat = [
            xDir[0], xDir[1], xDir[2], 0,
            yDir[0], yDir[1], yDir[2], 0,
            nDir[0], nDir[1], nDir[2], 0,
            cx + BEAT_SURFACE_OFFSET * nDir[0],
            cy + BEAT_SURFACE_OFFSET * nDir[1],
            cz + BEAT_SURFACE_OFFSET * nDir[2], 1,
        ];
        beatGroup.setMatrix(beatGroupMat);

        // Once calibrated, scatter beats and auto-start song 0 for testing.
        if (bothCalibrated && !beatsPositioned) {
            beatsPositioned = true;
            for (let s = 0; s < lyricNodes.length; s++) {
                for (let b = 0; b < lyricNodes[s].length; b++) {
                    lyricNodes[s][b].u = (Math.random() * 2 - 1) * 0.9; // [-0.9, 0.9]
                    lyricNodes[s][b].v = (Math.random() * 2 - 1) * 0.9;
                }
            }
            state.activeSongIdx = 0;
            state.started = true;
        }

        if (!bothCalibrated) return;
        
        beamL.update();
        beamR.update();

        // Animate cubes along z axis to match audio time if game started
        const audioTime = state.audioElement[state.activeSongIdx]
            ? state.audioElement[state.activeSongIdx].currentTime
            : 0;

        if (state.started && state.activeSongIdx !== null) {
            menuGroup.remove(0); // remove menu group from view

            // console.log('Animating lyrics for active song idx:', lyricNodes[state.activeSongIdx], 'audioTime:', audioTime.toFixed(2));
            for (let i = 0; i < lyricNodes[state.activeSongIdx].length; i++) {
                const nodeData = lyricNodes[state.activeSongIdx][i];
                const {
                    spawnTime,
                    beatTime,
                    u, v,
                    color: rgb,
                    hit,
                    node,
                } = nodeData;
                // Skip until positions have been assigned post-calibration
                if (u === null) continue;
                // console.log('node', nodeData)
                const MAX_SCALE = 0.05;
                let scaleFactor = 0;
                let opacity = 0;
                let color = [1, 1, 1];

                if (audioTime >= spawnTime && audioTime < beatTime + 1) {
                    state.currentBeatIdx = i;
                }

                if (hit) {
                    scaleFactor = 1;
                    opacity = 1;
                    color = rgb;
                } else if (audioTime < spawnTime) {
                    // not yet visible
                } else if (audioTime < beatTime) {
                    // grow 0→1 over the approach window
                    const t = (audioTime - spawnTime) / (beatTime - spawnTime);
                    scaleFactor = t;
                    opacity = t;
                    color = mixColor([1, 0.5, 0], t);
                } else if (audioTime < beatTime + 1) {
                    // full size for 1s after drop
                    scaleFactor = 1;
                    opacity = 1;
                    color = rgb;
                } else if (audioTime < beatTime + 1.5) {
                    // shrink out over 0.5s
                    const t = (audioTime - (beatTime + 1)) / 0.5;
                    scaleFactor = 1 - t;
                    opacity = 1 - t;
                    color = rgb;
                }

                node
                    .identity()
                    .move(u * w / 2, v * h / 2, 0)
                    .scale(MAX_SCALE * scaleFactor)
                    .opacity(opacity)
                    .color(color[0], color[1], color[2], opacity);
            }
        }

        for (let i = 0; i < menuGroup.nChildren(); i++) {
            const menu = menuGroup.child(i);

            // Default menu color at start of frame
            menu.color(0.1, 0.1, 0.1, 0.5);

            // Detect if beamRDisplay hits the menu using isPointInBox
            // Get the tip position of the beamRDisplay (z+ direction)
            // const isHit = cg.isPointInBox(beamLDisplay.position[0], beamLDisplay.position[1], beamLDisplay.position[2], menu.getGlobalMatrix());

            let hitMenu =
                beamR.hitRect(menu.getGlobalMatrix()) ||
                beamL.hitRect(menu.getGlobalMatrix());
            if (hitMenu) {
                menu.color(1, 1, 1);
                if (state.released) {
                    state.started = true;
                    console.log('Menu button released, starting game');
                }
                if (state.pressed) {
                    menu.color(1, 0.8, 0, 0.5);
                    // console.log('loading sound for song idx:', i, songs[i].url);
                    state.activeSongIdx = i; // set the active song based on their index
                    // console.log('Active song index set to:', state.activeSongIdx);
                    // Immediately pause and reset all other audio elements
                    for (let j = 0; j < state.audioElement.length; j++) {
                        if (j !== i) {
                            state.audioElement[j].pause();
                            state.audioElement[j].currentTime = 0;
                        }
                    }
                }
            }
        }

        // beam hitting on the beats
        const relevantNodes = lyricNodes[state.activeSongIdx] // get the current n last 5 beats for collision checking
            ? lyricNodes[state.activeSongIdx].slice(
                    Math.max(0, state.currentBeatIdx - 5),
                    state.currentBeatIdx + 1,
              )
            : [];
        if (lyricNodes[state.activeSongIdx]) {
            const PERFECT_WINDOW = 0.2; // seconds from beatTime for PERFECT
            let freshHitType = null; // 'perfect' or 'great'
            let freshHitPosition = null;
            let newlyMissed = false;

            for (let i = 0; i < relevantNodes.length; i++) {
                const nodeData = relevantNodes[i];
                const matrix = nodeData.node.getGlobalMatrix();
                let hit = beamL.hitRect(matrix) || beamR.hitRect(matrix);

                if (hit && !nodeData.hit) {
                    nodeData.hit = true;
                    const timingDiff = Math.abs(audioTime - nodeData.beatTime);
                    freshHitType = timingDiff <= PERFECT_WINDOW ? 'perfect' : 'great';
                    freshHitPosition = nodeData.position;
                    currentScore += freshHitType === 'perfect' ? 100 : 50;
                    console.log('Hit lyric idx:', i, currentScore);
                    // Play SFX from preloaded buffer
                    if (sfxBuffer && state.audioContext) {
                        const sfxSource = state.audioContext.createBufferSource();
                        sfxSource.buffer = sfxBuffer;
                        sfxSource.connect(state.audioContext.destination);
                        sfxSource.start(0);
                    }
                    // Remove previous score text (if any)
                    if (scoreboard.nChildren() > 1) {
                        scoreboard.remove(1);
                    }
                    // Add updated score text
                    scoreboard
                        .add(clay.text(currentScore.toString()))
                        .move(-0.5, 0.2, 0.01)
                        .scale(15);

                    if (model.time % 0.04 < 0.02) vibrate('right', 1, 20);
                }

                // Mark as missed only when the beat window expires without being hit
                if (!nodeData.hit && audioTime > nodeData.beatTime + 1 && !nodeData.missed) {
                    nodeData.missed = true;
                    newlyMissed = nodeData.position;
                }
            }

            // Update comment text once per frame based on what happened
            if (freshHitType === 'perfect') {
                if (commentText.nChildren() > 0) commentText.remove(0);
                commentText
                    .add(clay.text(COMMENT.PERFECT))
                    .move(freshHitPosition[0], freshHitPosition[1] + 0.1, LYRIC_Z)
                    .scale(COMMENT_TEXT_SCALE)
                    .color(0, 1, 0);
                state.commentTimer = model.time;
            } else if (freshHitType === 'great') {
                if (commentText.nChildren() > 0) commentText.remove(0);
                commentText
                    .add(clay.text(COMMENT.GREAT))
                    .move(freshHitPosition[0], freshHitPosition[1] + 0.1, LYRIC_Z)
                    .scale(COMMENT_TEXT_SCALE)
                    .color(1, 1, 0);
                state.commentTimer = model.time;
            } else if (newlyMissed) {
                if (commentText.nChildren() > 0) commentText.remove(0);
                commentText
                    .add(clay.text(COMMENT.MISSED))
                    .move(newlyMissed[0], newlyMissed[1] + 0.1, LYRIC_Z)
                    .scale(COMMENT_TEXT_SCALE)
                    .color(1, 0, 0);
                state.commentTimer = model.time;
            } else if (model.time - state.commentTimer > 1.0) {
                // Clear comment after 1 second
                if (commentText.nChildren() > 0) commentText.remove(0);
            }
        }

        // DEBUG
        // rect.color(0.25, 0.35, 0.5);
        // while (model.nChildren() > 1) {
        // 	model.remove(1);
        // }
        // LEFT CONTROLLER BEAM SHOWS TEXT OF U,V,D AND MAKES A STEADY VIBRATION

        // let uvdL = beamL.hitRect(rect.getGlobalMatrix());
        // if (uvdL) {
        // 	rect.color(1, 1, 1);
        // 	let u = uvdL[0],
        // 		v = uvdL[1],
        // 		d = uvdL[2];
        // 	let text = 'u:' + round(u) + '\nv:' + round(v) + '\nd:' + round(d);
        // 	model
        // 		.add(clay.text(text))
        // 		.move(-0.036, y + 0.03, 0.001)
        // 		.scale(0.02 / inch)
        // 		.color(0, 0, 0);
        // 	// vibrate('left', u * u < 0.033 && v * v < 0.09 ? 1 : 0.4);
        // }

        // // RIGHT CONTROLLER BEAM MOVES A TARGET OBJECT AND MAKES A PULSED VIBRATION

        // let uvdR = beamR.hitRect(rect.getGlobalMatrix());
        // if (uvdR) {
        // 	rect.color(1, 0.5, 0.5);
        // 	let u = uvdR[0],
        // 		v = uvdR[1];
        // 	model
        // 		.add('diskZ')
        // 		.move(0.2 * u, y + 0.1 * v, 0.001)
        // 		.scale(0.01)
        // 		.color(0, 0, 0)
        // 		.dull();
        // 	// if (model.time % 0.04 < 0.02) vibrate('right', 1, 20);
        // }

        let hitQuit =
            beamR.hitRect(quitBtn.getGlobalMatrix()) ||
            beamL.hitRect(quitBtn.getGlobalMatrix());
        if (hitQuit) {
            quitBtn.color(1, 0.5, 0.5);
            if (state.pressed) {
                state.started = false;
                state.activeSongIdx = null;
                currentScore = 0;
                state.currentBeatIdx = 0;

                // Reset all beat nodes across all songs
                for (let s = 0; s < lyricNodes.length; s++) {
                    for (let j = 0; j < lyricNodes[s].length; j++) {
                        lyricNodes[s][j].hit = false;
                        lyricNodes[s][j].missed = false;
                        lyricNodes[s][j].node
                            .identity()
                            .move(lyricNodes[s][j].position[0], lyricNodes[s][j].position[1], CUBE_START_Z)
                            .scale(0)
                            .opacity(0);
                    }
                }

                // Reset scoreboard text
                if (scoreboard.nChildren() > 1) scoreboard.remove(1);

                // Clear comment text
                if (commentText.nChildren() > 0) commentText.remove(0);

                // Re-add menu items
                for (let i = 0; i < songs.length; i++) {
                    const menu = menuGroup
                        .add('square')
                        .move(0, startY - i * gapY, -0.5)
                        .scale(0.2, 0.2, 0.01)
                        .color(0.25, 0.35, 0.5);
                    menu.add(clay.text(songs[i].title))
                        .move(-0.3, -0.05, 0.01)
                        .color(0, 0, 0)
                        .scale(6);
                }
            }
        } else {
            quitBtn.color(1, 0, 0);
        }

        // Play/pause music and run game logic based on state.started
        // Always ensure only the active song is playing
        for (let i = 0; i < state.audioElement.length; i++) {
            if (i === state.activeSongIdx) {
                if (state.started && state.audioElement[i].paused) {
                    state.audioElement[i].play();
                }
                if (!state.started && !state.audioElement[i].paused) {
                    state.audioElement[i].pause();
                    state.audioElement[i].currentTime = 0;
                }
            } else {
                if (!state.audioElement[i].paused) {
                    state.audioElement[i].pause();
                    state.audioElement[i].currentTime = 0;
                }
            }
        }
        if (!state.started) {
            return;
        }
    });
};
