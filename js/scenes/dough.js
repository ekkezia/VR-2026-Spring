...
export const init = async (model) => {
	...
	terrain.identity().move(-0.4, 1.5, 0).scale(0.4);
	...
	inputEvents.onDrag = (hand) => {
		// continous while pressing and moving
	...
	model.animate(() => {
		...
		handL
			.identity()
			.move(
				status.left.position[0],
				status.left.position[1],
				status.left.position[2],
			)
			.color(status.left.color[0], status.left.color[1], status.left.color[2])
			.scale(0.2);
		handR
			.identity()
			.move(
				status.right.position[0],
				status.right.position[1],
				status.right.position[2],
			)
			.color(
				status.right.color[0],
				status.right.color[1],
				status.right.color[2],
			)
			.scale(0.2);
		...
		try {
			...
			let diskNormalLocal = cg.normalize(
				cg.subtract(probeLocal, diskCenterLocal),
			);
			...
			for (let vInfo of vertices) {
				...
				if (Math.abs(distAlongNormal) <= planeDistEps) {
					...
					let proj = cg.subtract(
						vInfo.pos,
						cg.scale(diskNormalLocal, distAlongNormal),
					);
			...
			if (intersectVerts.length)
				console.log('intersections:', intersectVerts.length);
			...
			if (intersectVerts.length) {
				...
				avgMarker
					.identity()
					.move(avgGlobal[0], avgGlobal[1], avgGlobal[2])
					.scale(0.02)
					.color(0, 1, 0);
				console.log(
					'avg uv:',
