   ...
   model.animate(() => {
	 ...
	 vibrate('left', u*u < .033 && v*v < .09 ? 1 : .4);
      ...
      if (uvdR) {
         ...
         model.add('diskZ').move(.2*u,y+.1*v,.001).scale(.005).color(1,0,1).dull();
	 if (model.time % .04 < .02)
	    vibrate('right', 1, 20);
      }

      if (pressed) {
         const point = model.add('diskZ').scale(.001).color(1,1,0).dull();
         ...
         point.setMatrix(cg.mMultiply(rBeamMat, cg.mTranslate(0, 0, -0.3)));
         if (model.time % .04 < .02)
            vibrate('right', 1, 20);
      }
   });
